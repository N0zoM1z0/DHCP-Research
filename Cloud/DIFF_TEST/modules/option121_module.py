...
def test_option121(interface_name):
    """
    测试 DHCP Option 121 (Classless Static Route) 功能
    返回测试结果字典
    """
    ...
    try:
        ...
        # 请求 Option 121 (Classless Static Route)
        ...
        # --- 分析 DHCP OFFER 包 ---
        option121_found = False
        option121_content = None
        
        for opt in dhcp_offer[DHCP].options:
            if isinstance(opt, tuple) and opt[0] == 'classless-static-routes':
                option121_found = True
                option121_content = opt[1]
        ...
        dhcp_request = (
            ...
            DHCP(options=[
                ...
                ("server_id", server_ip),
                ("param_req_list", param_req_list),  # 再次请求 Option 121
                ("end")
            ])
        )
        ...
        # 检查ACK包中是否也有Option 121
        option121_in_ack = False
        option121_ack_content = None
        
        for opt in dhcp_ack[DHCP].options:
            if isinstance(opt, tuple) and opt[0] == 'classless-static-routes':
                option121_in_ack = True
                option121_ack_content = opt[1]
        
        return {
            "status": "成功",
            "assigned_ip": dhcp_ack[BOOTP].yiaddr,
            "server_ip": server_ip,
            "option121_support": "是" if option121_found else "否",
            "static_routes": str(option121_content) if option121_found else "N/A",
            "option121_in_ack": "是" if option121_in_ack else "否",
        }
